...
class SpeakerBrain(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        # uses softmax outside of its classifer
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        loss = self.hparams.compute_cost(predictions, command.squeeze(-1))

        if hasattr(self.hparams.lr_annealing, "on_batch_end"):
            self.hparams.lr_annealing.on_batch_end(self.optimizer)

        if stage != sb.Stage.TRAIN:
            self.error_metrics.append(uttid, predictions, command.squeeze(-1))
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.VALID:
            ...
            self.hparams.train_logger_tensorboard.log_stats(
                stats_meta={"epoch": epoch, "lr": old_lr},
                train_stats=self.train_stats,
                valid_stats=stage_stats,
            )
